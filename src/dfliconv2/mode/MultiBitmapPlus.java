	...
	protected MultiBitmapPlus(int w, int h, boolean plus) 
	{
		...
		for (int i = 0; i<w*h; i++)
		{
			for (int c = 0; c<8; c++)
			{
				...
				MultiPixels mp = null;
				if (plus)
					mp = new MultiPixels(new Add(new Const(x),xshift.get(y)),new Const(y),color0.get(y),color1[i],color2[i],color3.get(y),b01,b23,b45,b67);
				else
					mp = new MultiPixels(new Const(x),new Const(y),color0.get(0),color1[i],color2[i],color3.get(0),b01,b23,b45,b67);
				optiz.add(mp);
